    ...
    [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
        ...
        if (self.isAddLine) {
            if(idx !=  0){
                ...
                CGContextMoveToPoint(context, self.segmentWidth * idx, 2);
                CGContextAddLineToPoint(context, self.segmentWidth * idx,self.height-4);
        ...
        CGContextSetLineWidth(context,1.0);
        CGContextSetRGBStrokeColor(context, 0.5, 0.5, 0.5, 0.5);//线条颜色
        CGContextMoveToPoint(context, self.segmentWidth * (idx - 1), self.height-1);
        CGContextAddLineToPoint(context, self.segmentWidth * idx,self.height-1);
        CGContextStrokePath(context);
