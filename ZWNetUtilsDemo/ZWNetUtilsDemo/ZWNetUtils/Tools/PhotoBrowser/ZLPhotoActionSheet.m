...
- (instancetype)init
{
    ...
    if (self) {
        NSLog(@"[ZLPhotoTool getViewHeight] -> %ld",(long)[ZLPhotoTool getViewHeight]);
        NSLog(@"[[UIScreen mainScreen] bounds].size.height -> %f",[[UIScreen mainScreen] bounds].size.height);
        NSLog(@"kViewWidth -> %f",kViewWidth);
        if ([ZLPhotoTool getViewHeight] == 0) {
            [ZLPhotoTool saveViewHeight:[NSString stringWithFormat:@"%f",[[UIScreen mainScreen] bounds].size.height]];
        }
        self.frame = CGRectMake(0, 0, kViewWidth, [ZLPhotoTool getViewHeight]);
...
- (void)show
{
    ...
    if (self.animate) {
        CGPoint fromPoint = CGPointMake(kViewWidth/2, [ZLPhotoTool getViewHeight]+kBaseViewHeight/2);
        CGPoint toPoint   = CGPointMake(kViewWidth/2, [ZLPhotoTool getViewHeight]-kBaseViewHeight/2);
...
- (IBAction)btnCamera_Click:(id)sender
{
    if (self.arraySelectPhotos.count > 0) {
        ...
        [self cameraClickAction:sender];
    }
}

- (void)cameraClickAction:(UIButton *)sender{
...
{
    [self photoLibraryClickAction:sender];
}

- (void)photoLibraryClickAction:(UIButton *)sender{
...
- (void)presentVC:(UIViewController *)vc
{
    ...
    [nav.navigationBar setBackgroundImage:[self imageWithColor:kRGB(255, 212, 79)] forBarMetrics:UIBarMetricsDefault];
