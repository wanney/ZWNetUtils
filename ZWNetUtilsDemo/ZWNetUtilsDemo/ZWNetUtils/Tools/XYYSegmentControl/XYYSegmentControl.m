...
@property (nonatomic, strong) ZWSegmentedControl *ZWSegmentedControl;
...
{
    self.ZWSegmentedControl.selectionIndicatorColor = tabItemSelectionIndicatorColor;
}

-(void)setTabItemNormalBackgroundColor:(UIColor *)tabItemNormalBackgroundColor
{
    self.ZWSegmentedControl.backgroundColor = tabItemNormalBackgroundColor;//[UIColor colorWithRed:0.7 green:0.7 blue:0.7 alpha:1];
}

-(void)setTabItemNormalColor:(UIColor *)tabItemNormalColor
{
    _tabItemNormalColor = tabItemNormalColor;
    self.ZWSegmentedControl.titleTextAttributes = @{NSForegroundColorAttributeName : tabItemNormalColor} ;//@{NSForegroundColorAttributeName : [UIColor whiteColor]};
}

-(void)setTabItemSelectedColor:(UIColor *)tabItemSelectedColor
{
    _tabItemSelectedColor = tabItemSelectedColor;
    self.ZWSegmentedControl.selectedTitleTextAttributes = @{NSForegroundColorAttributeName : tabItemSelectedColor};//@{NSForegroundColorAttributeName : [UIColor orangeColor]};
}

-(void)setTabSelectionStyle:(XYYSegmentedControlSelectionStyle)tabSelectionStyle
{
    ZWSegmentedControlSelectionStyle HMSelectionStyle;
    if (tabSelectionStyle == ZWSegmentedControlSelectionStyleTextWidthStripe)
    {
        HMSelectionStyle = ZWSegmentedControlSelectionStyleTextWidthStripe;
        
    }else if (tabSelectionStyle == ZWSegmentedControlSelectionStyleFullWidthStripe)
    {
        HMSelectionStyle = ZWSegmentedControlSelectionStyleFullWidthStripe;
        
    }else if (tabSelectionStyle == ZWSegmentedControlSelectionStyleBox)
    {
        HMSelectionStyle = ZWSegmentedControlSelectionStyleBox;
        
    }else if (tabSelectionStyle == ZWSegmentedControlSelectionStyleArrow)
    {
        HMSelectionStyle = ZWSegmentedControlSelectionStyleArrow;
        
    }
    self.ZWSegmentedControl.selectionStyle = HMSelectionStyle;
}

-(void)setTabSelectionIndicatorLocation:(XYYSegmentedControlSelectionIndicatorLocation)tabSelectionIndicatorLocation
{
    ZWSegmentedControlSelectionIndicatorLocation selectionIndicatorLocation;
    if (tabSelectionIndicatorLocation == ZWSegmentedControlSelectionIndicatorLocationUp)
    {
        selectionIndicatorLocation = ZWSegmentedControlSelectionIndicatorLocationUp;
        
    }else if (tabSelectionIndicatorLocation == ZWSegmentedControlSelectionIndicatorLocationDown)
    {
        selectionIndicatorLocation = ZWSegmentedControlSelectionIndicatorLocationDown;
        
    }else if (tabSelectionIndicatorLocation == ZWSegmentedControlSelectionIndicatorLocationNone)
    {
        selectionIndicatorLocation = ZWSegmentedControlSelectionIndicatorLocationNone;
        
    }
    self.ZWSegmentedControl.selectionIndicatorLocation = selectionIndicatorLocation;
}
...
-(void)setTabItemNormalFont:(CGFloat)fontSize
{
    if (_tabItemNormalColor) {
        //设置了tab颜色
        self.ZWSegmentedControl.titleTextAttributes = @{NSFontAttributeName : [UIFont systemFontOfSize:fontSize],NSForegroundColorAttributeName : _tabItemNormalColor};
    }else
    {
        self.ZWSegmentedControl.titleTextAttributes = @{NSFontAttributeName : [UIFont systemFontOfSize:fontSize]};
    }
}
...
-(void)setTabItemSelectedFont:(CGFloat)fontSize
{
    if (_tabItemSelectedColor) {
        //设置了tab颜色
        self.ZWSegmentedControl.selectedTitleTextAttributes = @{NSFontAttributeName : [UIFont systemFontOfSize:fontSize],NSForegroundColorAttributeName : _tabItemSelectedColor};
    }else
    {
        self.ZWSegmentedControl.selectedTitleTextAttributes = @{NSFontAttributeName : [UIFont systemFontOfSize:fontSize]};
    }
}

-(void)setVerticalDividerEnabled:(BOOL)verticalDividerEnabled
{
    self.ZWSegmentedControl.verticalDividerEnabled = verticalDividerEnabled;
    if (verticalDividerEnabled) {
        self.ZWSegmentedControl.segmentEdgeInset = UIEdgeInsetsMake(0, 20, 0, 20);
    }
}

-(void)setVerticalDividerColor:(UIColor *)verticalDividerColor
{
    self.ZWSegmentedControl.verticalDividerColor = verticalDividerColor;
}

-(void)setVerticalDividerWidth:(CGFloat)verticalDividerWidth
{
    self.ZWSegmentedControl.verticalDividerWidth = verticalDividerWidth;
}
...
-(void)createImageTopView
{
    ...
    self.ZWSegmentedControl  = [[ZWSegmentedControl alloc] initWithSectionImages:sectionImagesArray
                                                           sectionSelectedImages:sectionSelectedImagesArray];
    self.ZWSegmentedControl.frame = CGRectMake(0, 0, viewWidth, kImagesHeightOfTopScrollView);
//    self.ZWSegmentedControl.autoresizingMask = UIViewAutoresizingFlexibleRightMargin | UIViewAutoresizingFlexibleWidth;
    self.ZWSegmentedControl.sectionTitles = _channelName;
    self.ZWSegmentedControl.selectedSegmentIndex = 0;
    //默认colors
...
-(void)changeSlideAtSegmentIndex:(NSInteger)index
{
    ...
    [self.ZWSegmentedControl setSelectedSegmentIndex:index animated:YES];
}
...
- (void)segmentedControlChangedValue:(ZWSegmentedControl *)segmentedControl {
